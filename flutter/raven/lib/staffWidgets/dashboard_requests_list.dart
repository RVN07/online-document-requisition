...
class _TableDashboardListState extends State<TableDashboardList> {
  ...
  _TableDashboardListState({required this.token});
...
final approvedRejectedRequests = documentList
 ...
 // approvedRejectedRequests.sort((a, b) => b.id.compareTo(a.id));
  ...
  return Column(
    ...
    children: [
      ...
      Expanded(
        flex: 3,
        child: DocumentAnalyticsOverlay(statusList: documentList, token: token),
      ),
      SizedBox(height: 6),
...
SizedBox(width: 10),]),
  ...
  columns: [
    ...
    const DataColumn(label: SizedBox( child: Center(child: Text('DETAILS', style: TextStyle(fontSize: 10)),),)),
          ...
          child: Row(
            children: [
              if (approved.status == 'approved')
                TextButton(
                  onPressed: () {
                    // Handle the action when the request is approved
                    ...
                    showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          ...
                          content: Text(
                            'is ${approved.firstName} ${approved.lastName} present and claimed this document request?'
                          ),
                          actions: [
                            TextButton(
                              onPressed: () {
                                claimDocumentRequest(
                                  token, approved.id as int);
                                fetchData();
                  ...
                  child: const Text('Claim Document'),
                ),
            ],
          ),
        ),
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  _buildChartCard(
                    'Approved / Rejected Requests Count',
  ...
  Widget _buildEmptyRequestsCard() {
    return Card(
      ...
      child: Container(
        ...
        child: const Center(
          child: Text(
            'No approved requests available for analytics.',
            style: TextStyle(
              fontSize: 18,
              fontWeight: FontWeight.bold,
              color: Colors.red,
